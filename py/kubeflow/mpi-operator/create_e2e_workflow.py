...
export KUBEFLOW_TESTING_REPO=/Users/niklashansson/Documents/OpenSource/testing
export KUBEFLOW_MPI=/Users/niklashansson/Documents/OpenSource/mpi-operator
export PYTHONPATH=${KUBEFLOW_MPI}/py:${KUBEFLOW_TESTING_REPO}/py
python kubeflowMPI/mpi_operator/e2e_tool.py apply --name=e2e-kfctl-test-$(date +%Y%m%d-%H%M%S)  --namespace=argo --dry-run=True --test_target_name=e2e_test
...
class Builder:
    def __init__(self, name=None, namespace=None, test_target_name=None,
        ...
        # The directory containing the kubeflow/mpi-operator repo
        self.src_dir = self.src_root_dir + "/kubeflow/mpi-operator"
        ...
        # The directory within the mpi-operator submodule containing
        # py scripts to use.
        self.kubeflow_mpijob_py = self.src_root_dir + "/kubeflow/mpi-operator/py"
    ...
    def _build_step(self, name, workflow, dag_name, task_template,
        ...
        # We extract the step from the argo workflow since we have a workflow where we like to add this step
    ...
    def build(self):
        ...
        #**************************************************************************
        # Run a dag of tests
        # [TODO] change here to the better....
        if self.test_target_name.startswith("e2e"):
            self._build_tests_dag_mpi_operator()
        else:
